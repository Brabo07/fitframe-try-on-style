// fitframe-try-on-style/src/components/ar/Glasses3DOverlay.tsx
...
const Scene = ({
  ...
  useEffect(() => {
    ...
    const centerX = faceLandmarks.noseBridge?.x ?? (faceLandmarks.leftEyeCenter.x + faceLandmarks.rightEyeCenter.x) / 2;
...
const Glasses3DOverlay = ({
  ...
  const frameColor = selectedProduct ? getFrameColorHex(selectedProduct.frame_color) : "#1a1a1a";
  const finalLensColor = selectedLensColor ? getLensColorHex(selectedLensColor) : (selectedProduct ? getLensColor(selectedProduct.frame_color) : "#4a90d9");
  const lensOpacity = selectedLensColor ? getLensOpacity(selectedLensColor) : 0.25;
